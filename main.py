...
def set_bkash(message):
    ...
    except: bot.reply_to(message, "⚠️ ফরম্যাট: /setbkash 01xxxxxxxxx")
...
def add_sellable(message):
    ...
    try:
        ...
        # স্টক প্যারামিটার বাদ দেওয়া হয়েছে
...
def add_item_admin(message):
    ...
    except: bot.reply_to(message, "❌ ফরম্যাট ভুল! ব্যবহার করুন: /additem Name|Price|Stock")
...
def show_sell(message):
    ...
    bot.send_message(message.chat.id, "কী বিক্রি করবেন? (কয় পিস তা পরে জানানো যাবে):", reply_markup=markup)
...
def finalize_sell(call):
    ...
    msg = bot.send_message(call.from_user.id, "কত পিস বিক্রি করবেন?")
    bot.register_next_step_handler(msg, get_sell_files)

def get_sell_files(message):
    try:
    ...
    except: bot.reply_to(message, "❌ দয়া করে একটি সঠিক সংখ্যা লিখুন।")

def save_sell(message):
    uid = message.chat.id
    name = pending_sells[uid]["name"]
    qty = pending_sells[uid]["qty"]
    ...
    total_price = sellable_types[name]['price'] * qty
    items[f"{name}_{uid}"] = {"price": total_price, "stock": qty, "f_id": f_id, "type": f_type, "text": text}
    user_balances[uid] = user_balances.get(uid, 0) + total_price
    bot.reply_to(message, f"✅ সেল সফল! {qty} পিস শপে যোগ হয়েছে। ব্যালেন্স: {total_price} BDT")

# --- DEPOSIT ---
@bot.message_handler(func=lambda m: m.text == "Deposit")
def deposit_menu(message):
    bot.send_message(message.chat.id, f"আমাদের বিকাশ নাম্বার: `{deposit_number}`\n\nটাকা পাঠানোর পর ট্রানজেকশন আইডি (TrxID) লিখুন:", parse_mode="Markdown")
...
def process_deposit(message):
    ...
    markup.add(types.InlineKeyboardButton("Approve", callback_data=f"app_{message.chat.id}"), 
               types.InlineKeyboardButton("Deny", callback_data=f"deny_{message.chat.id}"))
    bot.send_message(ADMIN_ID, f"🔔 নতুন ডিপোজিট রিকোয়েস্ট!\nইউজার: @{message.from_user.username}\nTrx: {trx}", reply_markup=markup)
    deposit_requests[message.chat.id] = {"trx": trx}
    bot.reply_to(message, "✅ রিকোয়েস্ট এডমিনের কাছে পাঠানো হয়েছে।")
...
def handle_deposit(call):
    ...
    if action == "app":
        user_balances[int(uid)] = user_balances.get(int(uid), 0) + 0 # এখানে অটো অ্যামাউন্ট বসানোর লজিক অ্যাড করা যায়
        bot.send_message(int(uid), "✅ আপনার ডিপোজিট এপ্রুভ হয়েছে!")
    else: bot.send_message(int(uid), "❌ আপনার ডিপোজিট রিজেক্ট হয়েছে।")
    bot.edit_message_text(f"✅ {action.upper()} করা হয়েছে।", call.message.chat.id, call.message.message_id)
